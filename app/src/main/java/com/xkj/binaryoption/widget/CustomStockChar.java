...
//            Log.i(TAG, "drawLink: w"+mWidth * (i + 1) / mMaxDataSize);
            pathLink.lineTo(mCharWidth * (i + 1) / mMaxDataSize, Double.valueOf(BigdecimalUtils.mul(BigdecimalUtils.sub(mMax, String.valueOf(mData.get(i + 1).getBid())), mHeightUnit)).intValue());
            //先这样，填充颜色后面做
//            pathFill.lineTo(mCharWidth * (i + 1) / mMaxDataSize, Double.valueOf(BigdecimalUtils.mul(BigdecimalUtils.sub(mMax, String.valueOf(mData.get(i + 1).getBid())), mHeightUnit)).intValue());
        }
